...
class SmartFocusApp(App):
    def __init__(self, **kwargs):
        ...
        self.frame_count = 0
        self.low_light_mode = False  # Low light mode flag
    ...
    def build(self):
        ...
        self.btn_lowlight = Button(
            text='🌙 Low-Light Mode',
            font_size='14sp',
            background_color=(0.8, 0.5, 0.2, 1),
            size_hint_x=0.25
        )
        self.btn_lowlight.bind(on_press=self.toggle_low_light)
        button_layout.add_widget(self.btn_lowlight)
        ...
        # Processing control buttons - Row 2
        ...
        self.status_label.text = "Ready - Click 'Download Video' or 'Use Webcam'"
        
        return self.layout
    
    def toggle_low_light(self, instance):
        """Toggle low-light enhancement mode"""
        self.low_light_mode = not self.low_light_mode
        if self.low_light_mode:
            self.btn_lowlight.background_color = (1, 1, 0, 1)  # Yellow when ON
            self.status_label.text = "Low-Light Mode: ON"
            self.instructions_label.text = "Enhanced for dark scenes"
        else:
            self.btn_lowlight.background_color = (0.8, 0.5, 0.2, 1)  # Orange when OFF
            self.status_label.text = "Low-Light Mode: OFF"
            self.instructions_label.text = ""
    
    def enhance_low_light(self, frame):
        """Enhance frame for low-light conditions"""
        if not self.low_light_mode or frame is None:
            return frame
        ...
        try:
            ...
            # Denoise for cleaner image
            enhanced = cv2.fastNlMeansDenoisingColored(enhanced, None, 10, 10, 7, 21)
            
            return enhanced
            
        except Exception as e:
            print(f"Low-light enhancement error: {e}")
            return frame
    ...
    def _download_video(self):
        ...
        try:
            video_path = r"C:\hackathon\sample_walking.mp4"
    ...
    def stop_processing(self, instance):
        ...
        self.btn_lowlight.disabled = False
